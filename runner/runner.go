...
// PrintError writes err to stderr prefixed with the program name, unless err
// is nil or an *exec.ExitError from a subprocess that inherited our stderr
// (it already printed its own diagnostics; repeating "exit status 1" would
// be noise). An ExitError produced by Output()-style capture instead carries
// the child's stderr on ExitError.Stderr — print that, since it was never
// shown. Call this before exiting with ExitCode so errors generated by
// dispatch itself (unknown subcommand, bad arguments, unsupported
// operations) are not silently swallowed.
func PrintError(name string, err error) {
	...
	if errors.As(err, &exitErr) {
		if len(exitErr.Stderr) > 0 {
			...
			if exitErr.Stderr[len(exitErr.Stderr)-1] != '\n' {
